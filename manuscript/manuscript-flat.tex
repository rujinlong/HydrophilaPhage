...
has selected for multidrug-resistant \emph{A.
hydrophila} strains and contributed
...
However, realizing this potential requires rigorous discovery and
characterization of suitable phage candidates.
...
Beyond delivering a candidate biocontrol agent, our integrated
nucleotide- and proteome-level analysis shows that Hp3 belongs to a
highly distinct, currently unclassified \emph{Caudoviricetes} genus,
indicating that the phages of this globally important aquaculture
pathogen remain substantially undersampled; coupling this genomic
novelty to a genomically defined safety profile and an \emph{in vivo}
survival benefit links phage discovery directly to therapeutic
assessment.
...
phase was prepared as the bacterial host. Phage Hp3 was added to the
bacterial suspension at MOIs of 0.001, 0.01, 0.1, 1.0, 10, 100, and 1000,
...
A one-step growth assay was performed to determine the latent period
...
Assay}{\emph{In Vitro} Lysis Assay}}\label{in-vitro-lysis-assay}
...
kit (cat. 46800) following the manufacturer's instructions. The concentration
...
classification, a proteomic network was constructed using vConTACT3 \citep{2025-Machine_Bolduc} against the NCBI viral RefSeq database (v230). The resulting network, which connects
...
Largemouth
Bass}{\emph{In Vivo} Therapeutic Efficacy in Largemouth Bass}}\label{in-vivo-therapeutic-efficacy-in-largemouth-bass}
...
intraperitoneally, followed by 0.1 mL of a suspension of phage Hp3 (9 ×
...
paraformaldehyde, embedded in conventional paraffin, and sectioned at 3 μm
...
vivo} study is reported in accordance with the ARRIVE guidelines (Animal
Research: Reporting of In Vivo Experiments). Unresolved reporting
limitations regarding power calculation, allocation procedures, and blinding
...
post-hoc test for pairwise comparisons. A \textit{P} value \textless{} 0.05 was
...
Hp3 formed small, round, transparent plaques with no lytic
halo around them on a double-layer agar plate
(Figure~\ref{fig-hp3-morphology} A). TEM showed that Hp3 possesses an
icosahedral head and a contractile tail, a morphology characteristic of
the class \emph{Caudoviricetes}. The head was approximately 81 nm in diameter, and the
tail was approximately 143 nm long and 23 nm wide.
...
strains (Table~\ref{tbl-host-range}). Hp3 had a narrow lytic
range, lysing only two of the 19 strains.
...
\subsection{Hp3 Has a Rapid Lytic Cycle and Is Stable Across a Range of Temperatures
and pH Values}\label{hp3-possesses-a-rapid-lytic-cycle-and-broad-environmental-stability}
...
study the optimal multiplicity of infection of phages, seven MOIs
(0.001, 0.01, 0.1, 1.0, 10, 100, and 1000) were tested. The results of the
...
The one-step growth curve showed a latent period of approximately 45
...
shown in Figure~\ref{fig-lysis-curve}, Hp3 showed a significant
...
23.191, \textit{P} \textless{} 0.001). All five MOIs suppressed bacterial density
relative to the untreated control (Tukey HSD, all \textit{P} \textless{} 0.005), with
MOI 1.0 and 10 producing the strongest and most sustained suppression (not
significantly different from each other, \textit{P} \textgreater{} 0.99, and
significantly stronger than MOI 100, \textit{P} \textless{} 0.05). In these
...
\caption{\label{fig-lysis-curve}48h lysis curve of Hp3. Five MOIs
(0.01, 0.1, 1.0, 10, and 100) of phage Hp3 were co-cultured with
\emph{A. hydrophila} at 37 °C; the control received bacteria without
phage. Bacterial density was monitored as the
...
= 23.191, \textit{P} \textless{} 0.001)}

\end{figure}%

\subsection{Genomic Analysis Places Hp3 in a Highly Distinct
\emph{Caudoviricetes}
Genus}\label{genomic-analysis-reveals-hp3-represents-a-putative-novel-viral-family}

Whole-genome sequencing showed that phage Hp3 has a
...
the same genus (Table~S5). Across all 200 complete
...
k-mers), consistent with the isolation of this genus. To test whether the
...
any recognized family approaching it (\mbox{Table~S4}; Figure~S3). Consistently, the vConTACT3
...
highly distinct genus. This genus-level assignment was independently
...
A direct comparative analysis of phage Hp3 with HJ05 showed a high
...
relationship (Figure~\ref{fig-genomics} C). A difference exists
...
lifestyle. It encodes a complete lysis cassette, comprising two holins (hp3\_0069 and
...
(CARD; Table~S2). The only virulence-database flag, hp3\_0022, arose solely from
...
rectangles in (A) and (C) mark a region of genomic difference
compared with HJ05.}
...
The \emph{in vivo} experimental design is illustrated in
...
survival over 7 days, indicating no acute mortality under this dose
and observation period. In contrast, the group infected with
...
0.026--0.320, \(P = 1.8 \times 10^{-4}\)). Full survival statistics are
given in Table~S3.
...
64.6, df = 3, \textit{P} \textless{} 0.001), and treatment improved survival
...
closest relative, \emph{Aeromonas} phage HJ05, in a highly distinct,
currently unclassified \emph{Aeromonas}-phage genus within the
\emph{Caudoviricetes}. This placement adds to accumulating evidence that
the phages of major aquaculture pathogens occupy under-explored regions
of viral sequence space, a diversity gap with direct consequences for how
biocontrol candidates are discovered, classified, and safety-screened. This placement should be read in the context of
...
antimicrobial-resistance genes or known virulence
determinants, providing a preliminary genomic safety basis for
therapeutic assessment.

However, the long-term efficacy of any phage therapy is governed by the co-evolutionary ``arms race'' between the phage and its
...
bacterial population observed in our \emph{in vitro} lysis assays is consistent
with the rapid emergence of resistant variants under phage selection
pressure. The durable \emph{in vivo} rescue despite this \emph{in vitro} regrowth may
...
Second, complementary explanation is an evolutionary trade-off: the
surface receptors that bacteria alter to resist phage infection are
frequently themselves virulence or colonization factors (such as
lipopolysaccharide, capsule, or flagella), so phage-resistant escape
mutants often pay a virulence cost \citep{2015-Virulence_Leon}. Under
this model, the resistant variants that regrow \emph{in vitro} may be
attenuated \emph{in vivo}, so that even partial bacterial killing
translates into a large survival benefit. We did not measure either
mechanism directly: tissue bacterial loads were not quantified as CFU/g,
and the virulence of phage-resistant isolates was not assessed, so
immune-mediated clearance and reduced virulence of escape mutants remain
plausible but untested explanations.
...
Beyond receptor specificity, the molecular basis of Hp3-host engagement
is itself a target for future work. The Hp3 receptor-binding tail fiber
(hp3\_0055) and its largest virion structural protein (hp3\_0096)
returned no significant sequence-level domain assignments and are
structurally divergent, consistent with the protein-level novelty of
this genus. \emph{Ab initio} structure prediction followed by structural
homology search nonetheless places the tail fiber within the
immunoglobulin-like (Ig-like) and fibronectin-III fold space that commonly
scaffolds phage receptor-binding proteins and underlies the
bacteriophage adherence-to-mucus (BAM) model of mucosal persistence
\citep{2013-Bacteriophage_Barr, 2006-IgLike_Fraser, 2024-Bacteriophage_Wu}.
Whether this Ig-like architecture promotes adherence to the fish mucosal
surface, and whether any Hp3 structural protein carries accessory
polysaccharide-depolymerase activity against the \emph{A. hydrophila}
capsule or biofilm, are testable hypotheses rather than established
mechanisms; our structural searches returned only weak and unconfirmed
signals for such depolymerase activity \citep{2016-Bacteriophageencoded_Pires,
2023-Potential_Guo}. Because \emph{A. hydrophila} forms biofilms as part
of its pathogenic lifestyle \citep{2024-Tad_Tekedar}, testing either
activity would refine our mechanistic understanding of Hp3-mediated
protection.
...
\citep{2024-Growing_Murtazalieva}. Our \emph{in silico} screen did not detect
...
Hp3 differs from this benchmark by representing a highly distinct
...
genome, divergent genus-level placement, lytic activity, and \emph{in vivo}
...
its placement in a highly distinct \emph{Aeromonas}-phage genus within
the \emph{Caudoviricetes}, with no known antimicrobial-resistance genes
or known virulence determinants. Hp3 administration increased
the survival rate from 15\% to 85\% in a largemouth bass infection
model, showing protective efficacy under the tested experimental
conditions. Hp3 is a promising but early-stage
...
\emph{in vivo} bacterial clearance.
...
\subsection{Declaration of interests}\label{competing-interests}
...
analysis: CL, YZ, JX and JR; Visualization: CL, YZ, ZC and JR;
...
Internationalization Training Project. This work was supported by the National Key Research and Development
