...
reader. Each MOI was assayed in three replicate wells and the uninfected
negative control in six wells.
...
viruses connected to Hp3 is presented (Figure~\ref{fig-genomics} B).
...
For phylogenetic analysis, the amino acid sequence of the terminase
large subunit was used as a marker. Homologous sequences were identified
across the \emph{Aeromonas} phage genome set, aligned with MAFFT
\citep{2013-MAFFT_Katoh}, and used to infer a maximum-likelihood tree in
IQ-TREE 2 \citep{2020-IQTREE_Minh} under the best-fit substitution model
selected by ModelFinder \citep{2017-ModelFinder_Kalyaanamoorthy}, with
branch support from 1,000 ultrafast bootstrap replicates. Comparative
...
\citep{2019-VFDB_Liu} databases, respectively.
...
as the reference. For bacterial growth (in
...
\(\mathrm{OD}_{600}\) absorbance. The data are presented as the mean ± SD (n = 3 per MOI group; n = 6 for the
untreated control).
...
double-stranded DNA genome of 84,750 bp with a GC content of 59\%
(Figure~\ref{fig-genomics} A). Bioinformatic analysis predicted a total
...
nucleotide-level, single-gene, and whole-proteome analyses. At the
...
the same genus. This relationship was strongly supported by a
maximum-likelihood phylogeny of the terminase large subunit, in which
Hp3 and HJ05 formed an exclusive, fully supported clade (ultrafast
bootstrap = 100; Figure~\ref{fig-genomics} C). Across all 200 complete
...
\emph{Straboviridae} (Figure~\ref{fig-genomics} B); we note, however,
...
relationship (Figure~\ref{fig-genomics} D). A notable difference exists
...
based on the terminase large subunit (TLS) protein. Phage Hp3 is
highlighted in red font. (D) Linear genome comparison of Hp3 and its
closest relative, \emph{Aeromonas} phage HJ05. Shaded regions connect
homologous genes with \textgreater30\% amino acid sequence identity. The
dashed rectangles in (A) and (D) highlight a region of genomic
difference compared with HJ05.}
...
relevant to aquaculture, and a genomic profile with no known
...
PRJNA1348731. The supplementary dataset supporting the findings of this
